...
#define ARR_SIZE 512
	...
	double stepSize = 2 * NG_PI / 50;
	for (int i = 0; i < ARR_SIZE; ++i) {
		arr[i] = std::sin(stepSize * (double(i)));
	}
	ngm::ComplexArray cArr(arr, ARR_SIZE);
	std::cout << ngm::FFT::outputCArrayReal(cArr, stepSize) << std::endl;
	ngm::FFT::transform(cArr);
	std::cout << ngm::FFT::outputCArrayAbs(cArr, stepSize) << std::endl;
	ngm::FFT::inverseTransform(cArr);
	std::cout << ngm::FFT::outputCArrayReal(cArr, stepSize) << std::endl;
